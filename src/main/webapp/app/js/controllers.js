...
myeltAnalyticsControllers
		.controller(
				...
				[
						...
						'$http',
						function($scope, $http) {
							var currentMonth = new Date(), lastMonth = new Date(), secondLastMonth = new Date(), thirdLastMonth = new Date(), fourthLastMonth = new Date(), fifthLastMonth = new Date();
							...
							thirdLastMonth
									.setMonth(currentMonth.getMonth() - 3);
							fourthLastMonth
									.setMonth(currentMonth.getMonth() - 4);
							fifthLastMonth
									.setMonth(currentMonth.getMonth() - 5);
							$scope.reports = {
								"uniqueUsers" : {
									"title" : "Accounts",
									"list" : [
								...
								"myeltUsageReport" : {
									...
									"list" : [
											...
											{
												"name" : monthNames[secondLastMonth
														.getMonth()]
														+ " '"
														+ secondLastMonth
																.getFullYear()
											...
											{
												"name" : monthNames[thirdLastMonth
														.getMonth()]
														+ "'"
														+ thirdLastMonth
																.getFullYear()
											...
											{
												"name" : monthNames[fourthLastMonth
														.getMonth()]
														+ " '"
														+ fourthLastMonth
																.getFullYear()
											...
											{
												"name" : monthNames[fifthLastMonth
														.getMonth()]
														+ " '"
														+ fifthLastMonth
																.getFullYear()
							...
							$scope.openReport = function(filename) {
								...
								var fullURL = "../reports/index.html#/dashboard/file/"
										+ filename;
								...
								$
										.fancybox({
											...
											afterShow : function() {
												...
												$(
														'<div class="fancybox-fullscreen"></div>')
														.appendTo(this.skin)
														.click(
																function() {
							...
							$('iframe').contents().bind("fullscreenerror",
									function() {
...
myeltAnalyticsControllers.controller('NavigationController', [ '$scope',
...
myeltAnalyticsControllers.controller('SettingsController', function($scope,
		jsonDataSetting) {
	$scope.mysqlInfo = jsonDataSetting.data;
	hideLoader();
});

myeltAnalyticsControllers.controller('RulesController', function($scope,
		jsonDataRules, jsonDataIgnoreInstitutions) {
	$scope.regionmap = jsonDataRules.data;
	$scope.ignoreinstitutions = jsonDataIgnoreInstitutions.data;
	hideLoader();
});

myeltAnalyticsControllers.controller('SyncController', function($scope, $http,
		$interval, jsonUserSyncData, jsonSubmissionSyncData) {
	$scope.usersJobInfo = jsonUserSyncData.data;
	$scope.submissionsJobInfo = jsonSubmissionSyncData.data;

	hideLoader();
	$scope.manageUsersSynButton = function() {
		if ($scope.usersJobInfo.jobStatus == "InProgress") {
			...
			if (fetchUserSyncPoller == null) {
				triggerFetchingUserSyncStatus();
			}
	...
	$scope.manageSubmissionSynButton = function() {
		if ($scope.submissionsJobInfo.jobStatus == "InProgress") {
			$scope.submissionsSpinClass = "fa-spin";
			$scope.submissionsStartButtonDisplay = "hide";
			$scope.submissionsResumeButtonDisplay = "hide";
			$scope.submissionsStopButtonDisplay = "";
			$scope.submissionsBadgeClass = "";
			if (fetchSubmissionSyncPoller == null) {
				triggerFetchingUserSyncStatus();
			}
		} else if ($scope.submissionsJobInfo.jobStatus == "Completed") {
			$scope.submissionsSpinClass = "";
			$scope.submissionsStartButtonDisplay = "";
			$scope.submissionsResumeButtonDisplay = "hide";
			$scope.submissionsStopButtonDisplay = "hide";
			$scope.submissionsBadgeClass = "badge-success";
			abortFetchingSubmissionSyncStatus();
		} else if ($scope.submissionsJobInfo.jobStatus == "Paused") {
			$scope.submissionsBadgeClass = "badge-error";
			$scope.submissionsSpinClass = "";
			$scope.submissionsStartButtonDisplay = "";
			$scope.submissionsResumeButtonDisplay = "";
			$scope.submissionsStopButtonDisplay = "hide";
		} else {
			$scope.submissionsBadgeClass = "badge-info";
			$scope.submissionsActiveClass = "active";
			$scope.submissionsStartButtonDisplay = "";
			$scope.submissionsResumeButtonDisplay = "hide";
			$scope.submissionsStopButtonDisplay = "hide";
		}
	...
	var fetchUserSyncPoller = null;

	triggerFetchingUserSyncStatus = function() {
		fetchUserSyncPoller = $interval(function() {
			$http.get("../sync/users/getSyncStatus").success(function(data) {
				$scope.usersJobInfo = data;
				$scope.manageUsersSynButton();
			})
		}, 5000);
	};

	abortFetchingUserSyncStatus = function() {
		$interval.cancel(fetchUserSyncPoller);
	};

	var fetchSubmissionSyncPoller = null;
	triggerFetchingSubmissionSyncStatus = function() {
		fetchSubmissionSyncPoller = $interval(function() {
			$http.get("../sync/submissions/getSyncStatus").success(
					function(data) {
						$scope.submissionsJobInfo = data;
						$scope.manageSubmissionSynButton();
					})
		}, 5000);
	};

	abortFetchingSubmissionSyncStatus = function() {
		$interval.cancel(fetchSubmissionSyncPoller);
	};

	$scope.startUserSync = function() {
	...
	$scope.startSubmissionSync = function() {
		...
		$http.get("../sync/submissions/start").success(function(data) {
			$scope.submissionsJobInfo = data;
			if ($scope.submissionsJobInfo.status == "success") {
				triggerFetchingSubmissionSyncStatus();
	...
	$scope.resumeSubmissionSync = function() {
		$("#submissionsStartButton").attr("disabled", "disabled");
		$("#submissionsResumeButton").attr("disabled", "disabled");
		$("#submissionsResumeButton i.fa-play-circle-o").hide();
		$("#submissionsResumeButton i.fa-spin").show();
		$("#userFailedRecord i.fa-repeat").hide();
		$http.get("../sync/submissions/resume").success(function(data) {
			$scope.submissionsJobInfo = data;
			if ($scope.submissionsJobInfo.status == "success") {
				triggerFetchingSubmissionSyncStatus();
				$("#submissionsStartButton").removeAttr("disabled");
...
myeltAnalyticsControllers
		.controller(
				"MyELTUsageReportController",
				function($scope, $routeParams, reportData) {
					...
					for ( var i = 0; i < 13; i++) {
						$scope.keys.push({
							"data" : monthNames[$scope.month - 1] + " '"
									+ $scope.year.toString().slice(-2)
						});
						if ($scope.month == 1) {
							$scope.month = 13;
							$scope.year--;
					...
					for ( var j = 0; j < $scope.keys.length; j++) {
						$scope.newStudentsReg_capes
								.push({
									"data" : $scope.usageReportData.new_student_registrations.CAPES[$scope.keys[j].data]
								});
						$scope.newStudentsReg_allOther
								.push({
									"data" : $scope.usageReportData.new_student_registrations.AllOther[$scope.keys[j].data]
								});
						$scope.newStudentsReg_total
								.push({
									"data" : $scope.usageReportData.new_student_registrations.CAPES[$scope.keys[j].data]
											+ $scope.usageReportData.new_student_registrations.AllOther[$scope.keys[j].data]
								});
						$scope.newProductsReg_capes
								.push({
									"data" : $scope.usageReportData.new_product_registrations.CAPES[$scope.keys[j].data]
								});
						$scope.newProductsReg_allOther
								.push({
									"data" : $scope.usageReportData.new_product_registrations.AllOther[$scope.keys[j].data]
								});
						$scope.newProductsReg_total
								.push({
									"data" : $scope.usageReportData.new_product_registrations.CAPES[$scope.keys[j].data]
											+ $scope.usageReportData.new_product_registrations.AllOther[$scope.keys[j].data]
								});
						if (j == $scope.keys.length - 2) {
							...
							$scope.activeUsers_total = $scope.usageReportData.active_user_accounts.CAPES[$scope.keys[j].data]
									+ $scope.usageReportData.active_user_accounts.AllOther[$scope.keys[j].data];
						}
					}
					hideLoader();
...
function showLoader() {
	...
	spinner.css("top", Math.max(0,
			(($(window).height() - spinner.outerHeight()) / 2)
					+ $(window).scrollTop())
			+ "px");
	spinner.css("left", Math.max(0,
			(($(window).width() - spinner.outerWidth()) / 2)
					+ $(window).scrollLeft())
			+ "px");
	spinner.show();
}
