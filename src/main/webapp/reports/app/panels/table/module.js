...
define("jquery-ui-dialog", function(){});
...
function (angular, app, _, kbn, moment) {
  ...
  module.controller('table', function($rootScope, $scope, $modal, $q, $compile, $timeout,
	...
	$scope.toggle_data_details = function(panel,data) {
      panel.showData = panel.showData ? false : true;
	  var wodata = $scope.without_kibana(data);
	  wodata = wodata.slice(panel.offset,panel.offset+panel.size);
	  if( panel.showData){
		$.fileDownload("/myeltanalytics/converToCSV", {
			preparingMessageHtml: "We are preparing your report, please wait...",
			failMessageHtml: "There was a problem generating your report, please try again.",
	          httpMethod: "POST",
	          data: JSON.stringify(wodata)
	      });
	  }
	  console.log(panel.showData);
      //row.kibana.view = row.kibana.view || 'table';
      //row.kibana.details = !row.kibana.details ? $scope.without_kibana(row) : false;
    };
