    ...
    public void run() {
        if(submissionsSyncService.jobInfo != null && !(submissionsSyncService.jobInfo.getJobStatus().equals(Helper.STATUS_PAUSED))){
            try
            {
                ...
                submissionsSyncService.jobInfo.setSuccessRecords(submissionsSyncService.jobInfo.getSuccessRecords() + 1);
                ...
                //e.printStackTrace();
                submissionsSyncService.jobInfo.setErrorRecords(submissionsSyncService.jobInfo.getErrorRecords() + 1);
                try
                {
                    submissionsSyncService.jobInfo.setLastId(submissionId);
                    submissionsSyncService.updateLastSyncedSubmissionStatus();
